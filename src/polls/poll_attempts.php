...
<div class="wrapper">
    ...
    foreach ($first_responses as $row) {
        ...
        if (!file_exists($_SERVER['DOCUMENT_ROOT']."/reports")) {
            mkdir($_SERVER['DOCUMENT_ROOT']."/reports", 0777, true);
        }
        $filename = 'report';
        if (isset($title) && isset($user_id) && isset($date) && isset($sum) && isset($max_points) && isset($indicator)) {
            ...
            $objWriter->save($_SERVER['DOCUMENT_ROOT']."/reports/" . $filename);
        }
        ...
        if (isset($title) && isset($user_id) && isset($date) && isset($sum) && isset($max_points) && isset($indicator)) {
            ...
            if ($counter == $counter2) {
                ...
                $objWriter->save($_SERVER['DOCUMENT_ROOT']."/reports/" . $filenameAll);
            }
        }
