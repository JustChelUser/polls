<?php
require_once $_SERVER['DOCUMENT_ROOT']."/src/common/functions.php";
checkAccessAdmin();
$fioErr = $loginErr = $passErr = $phoneErr= $emailErr='';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['save'])) {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            $sql = "SELECT COUNT(*) FROM parents WHERE email = :email AND id!=:id";
            $stmt = $pdo->prepare($sql);
        ...
        elseif ($flagValidation === 0)
        {
            $sql = "SELECT COUNT(*) FROM parents WHERE phone = :phone AND id!=:id ";
            $stmt = $pdo->prepare($sql);
        ...
        if ($flagValidation === 0) {
            ...
            $sql = "UPDATE parents SET Fio=:fio, email=:email, phone=:phone WHERE id=:id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute(['fio' => $fio, 'email' => $email, 'phone' => $phone, 'id' => $id]);
            if ($passwordChange===0){
...
// Получение ID учителя из параметра GET
if(isset($_GET['id']))
{
    $id = $_GET['id'];
}
if(isset($id))
{
    ...
    $pdo = connect();
    $sql = "SELECT p.id, p.id_user ,p.Fio,p.email, p.phone, u.login, u.password
        FROM parents p
...
<div class="wrapper">
    ...
    <form method="post">
        ...
        <input type="tel" id="phone" name="phone" required value="<?php if(isset($phone)) {echo $phone;} ?>">
