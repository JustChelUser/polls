...
<div class="wrapper">
    <main>
        <nav>
            <ul>
                ...
                <li><a href="NewStudent.php">Новый ученик</a></li>
            </ul>
        </nav>
    </main>
    ...
    $pdo = connect();
    $sql = "SELECT s.id, s.Fio, s.class_id, s.parent_id, c.name AS class_name, p.Fio AS parent_name
        FROM students s
    ...
    echo '<tr><th>ID</th><th>ФИО</th><th>Класс</th><th>Родитель</th><th>Действия</th></tr>';
    while ($row = $stmt->fetch()) {
        ...
        echo '<td><a href="EditStudentData.php?id=' . $id . '">Редактировать</a></td>';
        echo '</tr>';
    }
    echo '</table>';
    ?>
