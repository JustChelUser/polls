...
<div class="wrapper">
    <main>
        <nav>
            <ul>
                ...
                <li><a href="NewParent.php">Новый родитель</a></li>
            </ul>
        </nav>
    </main>
    ...
    $pdo = connect();
    $sql = "SELECT p.id,p.id_user, p.Fio,p.email,p.phone FROM parents p ORDER BY p.id";
    $stmt = $pdo->query($sql);

    // Отображение списка учеников и кнопки "Редактировать" для каждого ученика
    echo '<table class="table">';
    echo '<tr><th>ID</th><th>ФИО</th><th>Email</th><th>Телефон</th><th>Действия</th></tr>';
    while ($row = $stmt->fetch()) {
        ...
        echo '<td>' . $email . '</td>';
        echo '<td>' . $phone . '</td>';
        echo '<td><a href="EditParentData.php?id=' . $id . '">Редактировать</a></td>';
        echo '</tr>';
    }
    echo '</table>';
    ?>
