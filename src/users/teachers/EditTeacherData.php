...
checkAccessAdmin();
$fioErr = $classErr = $loginErr = $passErr = '';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['save'])) {
        ...
        if ($flagValidation === 0) {
            if ($class_id==='net')
            {
                // Обновление данных учителя в базе данных
                $sql = "UPDATE teachers SET Fio=:fio, class_id=:class_id WHERE id=:id";
                $stmt = $pdo->prepare($sql);
                $stmt->execute(['fio' => $fio, 'class_id' => NULL, 'id' => $id]);
            }
            else
            {
                // Обновление данных учителя в базе данных
...
if(isset($id))
{
    ...
    $pdo = connect();
    $sql = "SELECT t.id, t.Fio, t.class_id, t.id_user, c.name AS class_name, u.login
        FROM teachers t
    ...
    $row = $stmt->fetch();
    if (!$row) {
        // Если учителя с указанным ID не существует, перенаправляем на страницу со списком учителей
        header("Location: EditTeacher.php");
    }
}
...
<main>
    ...
    <span class="error"><?php echo $classErr; ?></span>
        ...
        <select id="class_id" name="class_id" required>
            ...
            $stmt_classes = $pdo->query("SELECT classes.id, classes.name
            FROM classes
