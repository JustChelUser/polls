...
<div class="wrapper">
    ...
    foreach ($first_responses as $row) {
        ...
        // Выполняем запрос для получения баллов для обычных ответов
        //на самом деле тут
        $sum = 0;
        foreach ($question_id_answer as $qia) {
            $sql = "SELECT answers.points FROM answers 
        WHERE question_id = :question_id AND answer = :answer LIMIT 1";
            $stmt = $pdo->prepare($sql);
        ...
        ///вот тут надо просто дату учесть
        foreach ($question_id_answer as $qia) {
        ...
        $filename = 'report';
        if (isset($title) && isset($fio) && isset($date) && isset($sum) && isset($max_points) && isset($indicator)) {
            ...
            $table->addCell(2000)->addText('Имя пользователя');
        ...
        if (isset($title) && isset($fio) && isset($date) && isset($sum) && isset($max_points) && isset($indicator)) {
            if ($flag === false) {
                ...
                $tableAll->addCell(2000)->addText('Имя пользователя');
            ...
            $rowAll->addCell(2000)->addText($fio);
        ...
        echo "<table class='table'><tr><th>Название опроса</th><th>Имя пользователя</th><th>Дата прохождения</th><th>Баллов набрано</th><th>Баллов максимум</th><th>Показатель</th><th>Скачать отчёт</th><th>Ответы пользователя</th><th>Удалить</th></tr>";
        echo "<tr><td>" . $title . "</td><td>" . $fio . "</td><td>" . $date . "</td><td>" . $sum . "</td><td>" . $max_points . "</td><td>" . $indicator . "</td><td><a href=/reports/$filename>Отчёт</a></td><td><a href='poll_response.php?poll_id=" . $poll_id . "&date=" . $date . "'>Ответы пользователя</a></td>";
